// Cloudflare Workers 兼容版本：使用 Web Crypto API 替代 node:crypto
// 注意：所有敏感配置必须通过环境变量传入，不要硬编码

// 管理员凭据（从环境变量读取，延迟初始化）
let ADMIN_USERNAME = "";
let ADMIN_PASSWORD = "";

// Secret for signing admin session tokens（从环境变量读取）
let TOKEN_SECRET = "";
...
export function initSecurity(env: {
  ...
  if (env.ADMIN_USERNAME) ADMIN_USERNAME = env.ADMIN_USERNAME;
  if (env.ADMIN_PASSWORD) ADMIN_PASSWORD = env.ADMIN_PASSWORD;
  if (env.ADMIN_TOKEN_SECRET) TOKEN_SECRET = env.ADMIN_TOKEN_SECRET;
  
  if (!ADMIN_USERNAME || !ADMIN_PASSWORD || !TOKEN_SECRET) {
    console.warn("[Security] Some credentials not configured via environment variables");
  }
...
/**
 * 确保安全配置已初始化（用于内部函数调用前检查）
 */
function ensureSecurityInit() {
  if (!securityInitialized) {
    console.warn("[Security] initSecurity() not called, using empty credentials");
  }
}
