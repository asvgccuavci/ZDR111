...
export async function seedAllStudents(): Promise<{ total: number; inserted: number; errors: number }> {
  ...
  if (totalSeed === 0) return { total: 0, inserted: 0, errors: 0 };
    ...
    for (const st of batch) {
      try {
        await query(
          ...
          [st.id, st.studentId, st.name, st.className, st.password, JSON.stringify(st.courses), true]
        );
        inserted++;
      } catch (e: any) {
        errors++;
        console.error(`[Seed] insert error for ${st.id}:`, e?.message || String(e));
      }
    }
  }
  console.log(`[Seed] imported ${inserted} students, ${errors} errors`);
  return { total: totalSeed, inserted, errors };
}
